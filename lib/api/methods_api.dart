...
class MethodsApi {
  ...
  MethodsApi([ApiClient apiClient]) : apiClient = apiClient ?? defaultApiClient;

  /// Add a tag to a document with HTTP info returned
  ///
  /// 
  Future<Response> apiMethodFrappeDeskTagsAddTagPostWithHttpInfo({ String tag, String dt, String dn }) async {
    Object postBody;

    // verify required params are set

    // create path and map variables
    String path = "/api/method/frappe.desk.tags.add_tag".replaceAll("{format}","json");
    ...
    if(dt != null) {
      queryParams.addAll(_convertParametersForCollectionFormat("", "dt", dt));
    }
    if(dn != null) {
      queryParams.addAll(_convertParametersForCollectionFormat("", "dn", dn));
    }
  ...
  ///
  /// 
  Future<Message> apiMethodFrappeDeskTagsAddTagPost({ String tag, String dt, String dn }) async {
    Response response = await apiMethodFrappeDeskTagsAddTagPostWithHttpInfo( tag: tag, dt: dt, dn: dn );
  ...
  /// Get the version of the app with HTTP info returned
  ///
  /// 
  Future<Response> apiMethodVersionGetWithHttpInfo() async {
    Object postBody;

    // verify required params are set

    // create path and map variables
    String path = "/api/method/version".replaceAll("{format}","json");
  ...
  ///
  /// 
  Future<Message> apiMethodVersionGet() async {
    Response response = await apiMethodVersionGetWithHttpInfo();
    if(response.statusCode >= 400) {
      throw ApiException(response.statusCode, _decodeBodyBytes(response));
    } else if(response.body != null) {
      return apiClient.deserialize(_decodeBodyBytes(response), 'Message') as Message;
